...
class Cherrybomb extends Enemy
{
    var explodeTimer = 1.010;
    
    var point:FlxSprite;

    var currentBombState = Normal;

    var cherrybombImage = FlxAtlasFrames.fromSparrow("assets/images/characters/cherrybomb.png", "assets/images/characters/cherrybomb.xml");

    public function new(x:Float, y:Float)
    {
        super(x, y);

        frames = cherrybombImage;
        animation.addByPrefix('walk', 'walk', 10, true);
        animation.addByPrefix('exploding', 'exploding', 5, true);
        animation.play('walk');
